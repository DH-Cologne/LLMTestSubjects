# Analyses of the completion answers from LLMs on sentences with ditransitive verbs (A1)
rm(list=ls())
...
# Replace "Auf [ABC]" with "[ABC]" 
replace_redundantanswers <- function(df) {
    df[] <- lapply(df, function(column1) {
      column1 <- gsub('Auf "(.*)"', "\\1", column1)
      return(column1)
    })
    return(df)
  }
...
# Read original Dataframes (A1: Ditransitive verbs, A2: Benefactive verbs)
