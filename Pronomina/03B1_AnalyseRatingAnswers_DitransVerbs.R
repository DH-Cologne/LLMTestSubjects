...
expBAnswers <- readRDS("Data/ExpB1DataAnswers.rds")
...
long_data <- pivot_longer(condensedData, cols = all_of(rating_columns), names_to = "Rating_Type", values_to = "Rating_Value")

# Calculate the number of NA values for each rating column
na_counts <- long_data %>%
  group_by(Rating_Type) %>%
  summarise(na_count = sum(is.na(Rating_Value)))

# Create boxplot
ggplot(long_data, aes(x = Rating_Type, y = Rating_Value)) +
  geom_boxplot(fill = "grey", color = "black", alpha = 0.7) +
  geom_text(data = na_counts, aes(x = Rating_Type, y = Inf, label = paste("NAs:", na_count)),
            vjust = -0.5, color = "red") +
  labs(
    title = "Boxplot of Different Ratings",
    x = "Rating Type",
    y = "Rating Value"
  ) +
  theme_minimal() +
